...
int read_from(int client_index, struct sockname *usernames) {
    ...
    if ((usernames[client_index]).username == NULL){
      char * username_on_heap = malloc(sizeof(char) * (BUF_SIZE + 1));
      int reading_username = read(fd, username_on_heap, BUF_SIZE);
      username_on_heap[reading_username] = '\0';
      (usernames[client_index]).username = username_on_heap;
      //strcpy(usernames[client_index]).username, username_on_heap);
      printf("When printing from read_from, the username is %s",\
      ...
      // Echo messages to all the other clients.
      for (int i = 0; i < MAX_CONNECTIONS; i++){
        if (i != client_index){ // Don't echo it to itself twice.
...
int main(void) {
    ...
    int max_fd = sock_fd;
    //fd_set shouldn't be white. Why is it white???
    fd_set all_fds, listen_fds;
    FD_ZERO(&all_fds);

    //Adds sock_fd to all_fds
        ...
        for (int index = 0; index < MAX_CONNECTIONS; index++) {
            if (usernames[index].sock_fd > -1 && FD_ISSET(usernames[index].sock_fd, &listen_fds)) {
                ...
                if (client_closed > 0) {
                    ...
                    printf("Echoing message from client %d, which has username %s \n", usernames[index].sock_fd, usernames[index].username);
                }
            }
        }
    }
